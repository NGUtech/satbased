...
final class AccountRepository implements RepositoryInterface
{
    ...
    public function persist(ProjectionInterface $account): bool
    {
        return $this->storageAdapter->write(
            (string)$account->getAccountId(),
            (array)$account->toNative()
        );
    }
    ...
    public function makeProjection(): Account
    {
        return Account::fromNative([EntityInterface::TYPE_KEY => Account::class]);
    }
}
